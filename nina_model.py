...
def tcn_block(x, filters, kernel_size=3, dilation_rate=1, dropout_rate=0.1):
    """ 
    优化后的 TCN 块 
    1. padding='same': 让卷积核能看到“未来”的信息，适合分类任务。
    2. SpatialDropout1D: 相比普通 Dropout，更适合时序特征，丢弃整个通道而不是独立的点。
    """
    prev_x = x
    
    # 1. 维度对齐 (如果通道数变了，或者 stride 变了，需要用 1x1 卷积调整 shortcut)
    if x.shape[-1] != filters:
        prev_x = layers.Conv1D(filters, 1, padding='same')(x)
        
    # --- 膨胀卷积层 1 ---
    # padding='same' 是核心修改，配合非因果任务
    x = layers.Conv1D(filters, kernel_size, padding='same', dilation_rate=dilation_rate)(x)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
    x = layers.SpatialDropout1D(dropout_rate)(x) # 使用 SpatialDropout
...
def build_tcn_model(input_shape, num_classes):
    """ 
    优化后的 TCN 模型 
    关键点：更大的 Kernel Size 以覆盖 300ms 的窗口。
    """
    inputs = layers.Input(shape=input_shape)
    ...
    # 堆叠 TCN 块
    # 建议将 Kernel Size 设为 5 或 7
    k_size = 5 
    
    x = tcn_block(x, 64,  kernel_size=k_size, dilation_rate=1,  dropout_rate=0.1)
    x = tcn_block(x, 64,  kernel_size=k_size, dilation_rate=2,  dropout_rate=0.1)
    x = tcn_block(x, 128, kernel_size=k_size, dilation_rate=4,  dropout_rate=0.1)
    x = tcn_block(x, 128, kernel_size=k_size, dilation_rate=8,  dropout_rate=0.1)
    # 如果窗口特别长，可以继续加 dilation=16，或者在中间加 MaxPooling
    x = tcn_block(x, 256, kernel_size=k_size, dilation_rate=16, dropout_rate=0.1)
...
def build_dual_stream_model(input_shape, num_classes):
    ...
    # 2. 计算 STFT
    # frame_length=256, frame_step=32 对应约 256ms 窗口
    stft = layers.Lambda(lambda x: tf.signal.stft(x, frame_length=128, frame_step=32))(merged_channel)
